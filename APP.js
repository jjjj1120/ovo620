...
8. 强制独立思考是否贴合人设，是否做到了要求的不油腻等等条件，独立思考结束后才允许输出。\n`;
    ...
    const loadSettings=()=>{
        ...
        customizableIcons.forEach(item=>{
            ...
            if(savedIcon)applyCustomIcon(item.id,savedIcon);
    ...
    if (igEditProfileBtn) {
        igEditProfileBtn.addEventListener('click', () => {
            const igData = JSON.parse(localStorage.getItem('ig_profile_data') || '{}');
            const lineData = JSON.parse(localStorage.getItem('line_profile_data') || '{}');
    ...
    if (uploadIgAvatar) {
        uploadIgAvatar.addEventListener('change', (e) => {
            const file = e.target.files[0];
            if (!file) return;
            compressImage(file, 400, 400, 0.8, (dataUrl) => {
                if (dataUrl) {
    ...
    if (saveIgProfileBtn) {
        saveIgProfileBtn.addEventListener('click', () => {
            const igData = {
